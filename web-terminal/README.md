...
`web/` 만 바뀌었으면 **새로고침**으로 끝납니다(정적 파일에 캐시를 두지 않습니다).
`pyserver/` 가 바뀌었으면 콘솔에서 `Ctrl+C` 로 끄고 다시 실행해야 합니다.
헷갈리면 껐다 켜면 됩니다 — 단, 서버를 끄면 열려 있던 터미널 세션도 함께 종료됩니다.
...
| 글꼴·줄 간격 | 탭 바의 **가 글꼴** — 설치 여부를 확인해 표시 |
| 복사 | 선택 후 `Ctrl+C` (선택이 없으면 `Ctrl+C` 는 그대로 중단 신호) |
| 붙여넣기 | `Ctrl+V` · 터미널 우클릭 → 붙여넣기 |
| 터미널 우클릭 | 복사 / 붙여넣기 / 전체 선택 / 찾기 / 화면 지우기 |
| 검색 | `Ctrl+F` |
